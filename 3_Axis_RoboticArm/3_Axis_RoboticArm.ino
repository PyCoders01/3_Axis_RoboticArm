...
int sensorPin2 = A2;   
int sensorPin3 = A3;   
int count0, arrayStep, arrayMax, countverz, press_tester, stepsMax, steps, time = 1000, del = 1000, temp;
unsigned int  verz = 0;
...
void setup()
{
  ...
  servo_0.attach(2); // attaches the servo
  servo_1.attach(3);
  servo_2.attach(4);
  servo_3.attach(7);
...
void Button() // check buttons for single and doubleclick
{
  ...
  {
    delay(20);
    if (digitalRead(10) == true) // press_tester losgelassen
    {
      if (press_tester == 0)
      {
        press_tester = 1;
        previousMillis3 = currentMillis;
        //Serial.print("Status Record "); Serial.println(press_tester); 
      }
      else if ((press_tester == 1) && (currentMillis - previousMillis3 < 5000))
      {
        press_tester = 2;
        //Serial.println(press_tester); 
      }
      /*else if ((press_tester == 2) && (currentMillis - previousMillis3 < 500))
      {
        press_tester = 3;
        Serial.println(press_tester); 
      }*/
    ...
    if ((press_tester == 1) && ((currentMillis - previousMillis3) > 1000)) // write to array
    {
      arrayStep += 1;
      arrayMax = arrayStep;
      record();
